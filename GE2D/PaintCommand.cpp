...
PaintCommand::PaintCommand(MapEditor& mapEditor, Vector2D position, const std::string& paintType)
...
bool PaintCommand::operator==(const Command& other) const
{
    const PaintCommand* otherCommand = dynamic_cast<const PaintCommand*>(&other);
    if (!otherCommand) {
        return false;
    }
